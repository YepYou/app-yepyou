...
const Missions = () => {
  ...
  async function getMissions() {
    ...
    if (netinfo.isConnected) {
      ...
      const {data: missionsData} = await api.get(
        `/v1/missions?active=true&user=${user.id}&world=${route.params.world._id}`,
      );
  ...
  if (!haveConnetion) {
    return <WithoutConnection getData={getMissions} />;
  }
      ...
      <TouchableOpacity
        ...
        <Layout style={styles.cardMissionBlockContent}>
          <Layout style={styles.cardMissionBlockContentBar}>
            <Text style={styles.cardMissionBlockContentBarText}>
      ...
      <Layout style={styles.container}>
        <Modal backdropStyle={styles.backdropModal} visible={modalVisible}>
          <Card disabled={true}>
            ...
            <Text>
              Para aceitar esta missão você vai precisar de uma conta Premium!
              🚀
            </Text>
